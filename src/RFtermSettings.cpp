...
void CRFtermSettings::SetTextWrapping(TBool aState, TBool aNotify)
	{
	iWrapText = aState ? 1 : 0;
	if (aNotify)
		Notify();
	}
...
void CRFtermSettings::LoadL(RReadStream& aStream)
	...
	SetTextWrapping(aStream.ReadInt8L(), EFalse);
